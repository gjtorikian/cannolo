 ...
 *  Parsers.cannolo(Cannolo) -> Void
 *
 *  Registers Cannolo parser as `cannolo`.
 *
 *
 *  ##### Example
 *
 *      Cannolo.parse(files, options, function (err, ast) {
 *        // ...
 *      });
 **/
...
// internal
var parser = require('./pdoc/pdoc');
var Cannolo = require(__dirname + '/../../../cannolo');
...
var process_javascript = function(source, file, options, callback) {
  ...
  parser.parser = Cannolo.setParsingRules(options, parser.parser);
...
module.exports = function (CannoloArg, args) {
  if (!args || !args.registered) { // for horrendous markdown support
    //if (args) console.log(args);
    CannoloArg.registerParser('.js', parse_javascript);
  }
  else {
