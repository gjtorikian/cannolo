...
// internal
var template    = require('./cannolo/common').template;
var renderers   = require('./cannolo/renderers');
var parsers     = require('./cannolo/parsers');

var Cannolo = module.exports = {};
  ...
  _.each(nodes, function (node, id) {
    ...
    function buildNodeRecursive(node, path, inheritedMembers)
    {
        ...
        if (nodes[path] !== undefined && nodes[path].inherits) {
          inheritedMembers.push({id: path, children: addInheritedFrom(path, inheritedChildren)});
...
/**
 *  Cannolo.parse(files, options, callback) -> Void
 ...
 **/
Cannolo.parse = function parse(paths, buildOptions, callback) {

  var options = Cannolo.cli.parseArgs(paths);
  ...
  if (options.parseType == "jsd")
    Cannolo.use(require(__dirname + '/cannolo/plugins/parsers/jsd_parser'));
  else 
    Cannolo.use(require(__dirname + '/cannolo/plugins/parsers/pdoc_parser'));
  
  Cannolo.use(require(__dirname + '/cannolo/plugins/parsers/markdown'));

  //
  // Process aliases
  //

  Cannolo.cli.findFiles(options.paths, options.exclude, function(err, files) {
    if (err) {
      console.error(err);
      process.exit(1);
    }
...
/**
 *  Cannolo.render(name, ast, buildOptions, callback) -> Void
 ...
 **/
Cannolo.render = function render(name, ast, buildOptions, callback) {
  if (!renderers[name]) {
    callback("Unknown renderer: " + name);
    return;
  }

  var options = Cannolo.cli.parseArgs(["blank"]);
 ...
 *  Cannolo.cli -> cli
 **/
Cannolo.cli = require('./cannolo/cli');

/**
 *  Cannolo.VERSION -> String
 *
 *  Cannolo version.
 **/
Cannolo.VERSION = require('./cannolo/version');


/**
 *  Cannolo.use(plugin) -> Void
 *  - plugin (Function): Infection `plugin(CannoloClass)`
 *
 *  Runs given `plugin` against Cannolo base class.
 *
 *
 *  ##### Examples
 *
 *      Cannolo.use(require('my-renderer'));
 **/
Cannolo.use = function use(plugin) {
  plugin(this);
};


/**
 *  Cannolo.registerRenderer(name, func) -> Void
 ...
 **/
Cannolo.registerRenderer = function (name, func) {
  renderers[name] = func;
};


/**
 *  Cannolo.registerParser(extension, func) -> Void
 *  - extension (String): Extension suitable for the parser, e.g. `'js'`
 *  - func (Function): Parser function `func(source, options, callback)`
 *
 *  Registers given function as `name` renderer.
 **/
Cannolo.registerParser = function (extension, func) {
...
Cannolo.setParsingRules = function(options, parser) {
  ...
  if (options.parseOptions !== undefined && options.parseOptions !== null) {
      ...
      parseOptionsJSON = JSON.parse(fs.readFileSync(__dirname + "/cannolo/plugins/parsers/javascript/pdoc/defaultParseRules.json"));
  }
...
/**
 *  Cannolo.extensionAlias(alias, extension) -> Void
 ...
 *
 *  ``` javascript
 *  cannolo.extensionAlias('cc', 'js');
 *  ```
 *
 *
 *  ##### See Also
 *
 *  - [[Cannolo.registerParser]]
 **/
Cannolo.extensionAlias = function (alias, extension) {
...
//
Cannolo.use(require(__dirname + '/cannolo/plugins/renderers/html'));
Cannolo.use(require(__dirname + '/cannolo/plugins/renderers/json'));
Cannolo.use(require(__dirname + '/cannolo/plugins/renderers/c9ac'));